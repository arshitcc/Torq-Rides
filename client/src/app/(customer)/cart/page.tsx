...
export default function CartPage() {
  ...
  const calculateItemBreakup = (item: CartItem) => {
    const days = differenceInDays(new Date(item.dropoffDate), new Date(item.pickupDate)) + 1
  ...
  const handlePayment = async (e: React.FormEvent) => {
    try {
      ...
      const amount = paymentMethod === "partial" ? calculateAdvancePayment() : cart?.discountedTotal

      const order = (await generateRazorpayOrder(paymentMethod === "partial" ? "p" : "f")) as unknown as {
        id: string
      }
          ...
          try {
            ...
            if (paymentResponse) {
              ...
              setBookingDetails({
                ...
                amount: amount,
                paymentMethod: paymentMethod === "partial" ? "Partial Payment" : "Full Payment",
                motorcycles: cart.items.map((item) => ({
                  ...
                  dropoffDate: format(new Date(item.dropoffDate), "MMM dd, yyyy"),
                })),
              })
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-semibold">
                    ₹{paymentMethod === "partial" ? calculateAdvancePayment() : cart?.discountedTotal}
                  </span>
                </div>
                <div className="flex justify-between text-sm">
          ...
          <CardContent className="p-8">
            <div className="text-center space-y-6">
              ...
              <div>
                ...
                <p className="text-gray-600 dark:text-gray-300">
                  Your motorcycle rental has been successfully booked. You'll receive a confirmation email shortly.
                </p>
              </div>
                ...
                <CardContent className="p-6">
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-bold text-green-600">#{bookingDetails.bookingId}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="font-medium">Amount Paid:</span>
                      <span className="font-bold">₹{bookingDetails.amount.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="font-medium">Payment Type:</span>
                      <span className="font-bold">{bookingDetails.paymentMethod}</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
                    ...
                    <div className="flex items-start space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-yellow-700 dark:text-yellow-300">
                          Please pay the remaining amount of ₹{(cart?.discountedTotal || 0) - bookingDetails.amount} at
                          the time of pickup.
                        </p>
                      </div>
                    </div>
        ...
        <Card className="w-full max-w-md mx-4">
          <CardContent className="p-8">
            <div className="text-center space-y-6">
              ...
              <div className="space-y-3">
                <Button onClick={resetToCart} className="w-full bg-primary hover:bg-primary/90">
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="text-center space-y-6">
          ...
          <p className="text-gray-600">Add some motorcycles to your cart to get started.</p>
          <Button asChild className="bg-yellow-primary hover:bg-yellow-600 text-black">
            <Link href="/bikes">Browse Motorcycles</Link>
          </Button>
        </div>
      </div>
    )
      ...
      <div className="container mx-auto px-4 py-8 space-y-4">
        ...
        <div className="flex flex-row items-center justify-between mb-6">
          <div className="flex items-center space-x-4">
            <h1 className="text-3xl font-bold ml-4">Cart ({cart?.items?.length})</h1>
          </div>
          <Button
            variant="outline"
                      ...
                      <div className="flex-1 p-4">
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-4">
                          <div>
                            ...
                            <p className="font-medium">{format(new Date(item.pickupDate), "MMM dd, yyyy")}</p>
                            <p className="text-xs text-gray-500">{item.pickupTime}</p>
                          </div>
                          <div>
                            <p className="text-sm text-gray-500">Drop Off Date</p>
                            <p className="font-medium">{format(new Date(item.dropoffDate), "MMM dd, yyyy")}</p>
                        ...
                        <div className="mb-4">
                          ...
                          <ul className="text-sm text-gray-600 space-y-1">
                            ...
                            <li>• ₹2000 deposit will be refunded within 2 days after drop-off.</li>
                        ...
                        <div className="flex justify-between items-center pt-4 border-t">
                          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 items-center space-x-4">
                            <div className="text-center">
                              ...
                              <p className="font-semibold">₹{item.motorcycle.rentPerDay * item.quantity}</p>
                            </div>
                            <div className="text-center">
                              <p className="text-sm text-gray-500">Security Deposit</p>
                              <p className="font-semibold">₹{item.motorcycle.securityDeposit * item.quantity}</p>
                            </div>
                            <div className="text-center">
              ...
              <CardContent className="space-y-6">
                  ...
                  {cart.coupon ? (
                    <div className="flex items-center justify-between p-3 bg-green-50 border border-green-200 rounded-lg">
                      <div className="flex items-center space-x-2">
                        ...
                        <div>
                          <p className="font-medium text-green-800">{cart.coupon.promoCode}</p>
                    ...
                    <div className="flex space-x-2">
                      ...
                      >
                        {applyingCoupon ? <Loader2Icon className="w-4 h-4 animate-spin" /> : "Apply"}
                      </Button>
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-3">
                  ...
                  {cart?.couponId?.toString().length > 0 && (
                    <div className="flex justify-between">
                      <span>Discount</span>
                      <span className="font-semibold text-red-500">- ₹{cart.cartTotal - cart.discountedTotal}</span>
                    </div>
                  )}
                  <Separator />
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <Label htmlFor="full" className="flex-1">
                        <div>
                          ...
                          <p className="text-sm text-gray-500">Pay full amount ₹{cart.discountedTotal}</p>
                        </div>
                      </Label>
                    </div>
                  </RadioGroup>
                ...
                <div className="p-4 bg-yellow-50 rounded-lg border border-yellow-200">
                  ...
                  {paymentMethod === "partial" && (
                    <p className="text-sm text-gray-600 mt-1">
                      Pay remaining Rent + Deposit (₹{cart.discountedTotal - calculateAdvancePayment()}) at the time of
                      pickup
                    </p>
                  )}
                </div>
                ...
                >
                  Pay ₹{paymentMethod === "partial" ? calculateAdvancePayment() : cart.cartTotal} and Reserve
                </Button>

                {/* Payment Methods */}
                <div className="pt-4 border-t">
                  <h4 className="font-medium mb-3 text-center">100% Secure Payment By</h4>
                  <div className="mt-6 pt-6 border-t">
                    <h4 className="font-medium mb-3 text-center">Accepted Payment Methods</h4>
